...
在原版基础上新增了 **耕种 / 地图传送 + 汽车传送 / 载具导航模式(沿道路网自动行驶) / 点亮全图 / 真-夜视 / 战斗强化 + 临时武器 / 战利品重掷 / ESP / 物品雷达 + 小地图标记** 等大量功能, 并针对 B42 客户端 Lua 环境 (Kahlua) 做了多处修复与加固。完整功能见下方「功能一览」。
...
- 反作弊状态(权限反作弊 / 移动反作弊 / 自定义日志系统 / BikiniTools 可用性)、在线玩家名单(含隐身标记)、注意事项、作者与联系方式

### 生存

- **物品与携带**: 手中物品无限耐久 / 自动修理背包中的物品 / 无限负重
...
- **刷弹药**: 按弹匣/枪械类型生成弹药
- **钓竿生成**: 主手装备鱼竿后可将列表中任意物品生成进背包(约 5~8 秒入包, 仅多人)
