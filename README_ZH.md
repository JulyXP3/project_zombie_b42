...
在原版基础上新增了 **耕种 / 地图传送 + 汽车传送 / 载具导航模式(沿道路网自动行驶) / 点亮全图 / 真-夜视 / 战斗强化 + 临时武器 / 战利品重掷 + 物品生成(物品生成 (MP) · 尸体生成 · 计时生成 · 开盒生成) / ESP / 物品雷达 + 小地图标记** 等大量功能, 并针对 B42 客户端 Lua 环境 (Kahlua) 做了多处修复与加固。完整功能见下方「功能一览」。
...
- **调试权限功能(仅单人)**: 上帝模式 / 穿墙模式 / 隐身模式 / 创造模式 / 进度条秒走完 —— 需先在「其他」页开启「解锁调试权限(单人)」
- **特殊模式**: 夜视模式 / **真-夜视**(渲染级全亮, 夜色与视野锥黑幕移除, 无灯室内不再漆黑, 纯客户端) / 僵尸不会攻击玩家(多人可用)

### 战斗

- **战斗强化**: 攻速倍率(1~2.5) / 攻击距离加成(0~4) / 秒杀模式 / 暴击Max / 枪械只爆头(必爆头, 3 倍伤害) / 枪械百发百中(无视环境) / 群攻 / 提高枪械射速 / 无限弹药(耗尽自动补满; 补充数量在「战利品」页设置) / 无卡壳
- **超级群攻**: 每次挥击命中上限 10~20(可攻击以自身为中心的全部敌人; 原版的多目标伤害递减已取消, 每只都吃全额伤害, 可配合暴击Max/秒杀模式)
- **临时武器**: 本地生成枪械替换手持(全枪械列表搜索 + 临时替换/取消替换)

### 物品(SP)

- **物品创建**: 名称/ID 过滤, 一键给予 x1/x2/x5/x10
...
- 用背包中的**衣物/背包**当祭品, 一件一换指定目标物品(目标表带名称/ID 搜索, 祭品列表一键刷新)

### 玩家

- **玩家信息 & 配方**: 编辑存活时间 / 编辑击杀僵尸数 / 学习所有可用的制作配方
- **VHS教学(仅多人)**: 可训练技能列表(搜索) + 升级所选技能 / 全技能升级(需附近有开着且音量 5 以上的收音机/电视/车载收音机, 三种都没有时自动把背包里的收音机摆出来; 每技能 30 秒冷却, XP 上限默认 3 级随服务器配置而定)
- **角色特性**: 特性添加/移除; **技能**: 技能等级 ± / 添加经验 / 所有技能升满

### ESP

- 总开关 + 四模块: 玩家信息(附近玩家用户名、主/副手物品)、载具信息(马力/极速)、僵尸信息(头顶 HP 显示、僵尸雷达)、独立功能(玩家雷达 150 格、载具雷达、视野 360 度 —— 所有僵尸/载具/玩家强制显形)
...
- **开盒生成**: 背包备好载体(玻璃罐整盒/双三四原木堆/柴捆), 选择目标与数量后点击生成, 再正常做一次对应开盒/拆堆合成即放出列表中任意物品(仅多人)
- **免费建造**: 不消耗材料直接建墙 (背包与脚下不能有木板钉子, 否则会被正常扣除; 仅多人)
...
- 解锁调试权限(单人) / 服务器同步保护(防止属性与技能被服务端回滚) / 管理员菜单与尝试获取管理员权限 / 调试菜单(主)·游戏调试菜单·车辆机械菜单·医疗菜单 / 发放所选配方的所有材料 / 禁止执行默认日志记录器 / 禁止执行提及作弊的文件 / 禁止执行含可疑词的文件

### 设置

- 界面语言(中/英/俄) / 按键绑定(独立子面板) / 强调色 / 玩家·载具·僵尸 UI 元素渲染颜色 / 配置列表(保存/加载/删除/重置默认) / 重载所有 Lua 元素 / 重置默认设置
