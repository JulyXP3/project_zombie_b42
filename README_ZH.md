...
- 解锁调试权限(单人) / 尝试获取管理员权限(12型) / 游戏调试菜单 / 管理员菜单(8型) / 车辆机械菜单 / 医疗菜单 / 学习所有可用的制作配方 / 发放所选配方材料 / 攻速倍率(1.0-3.0, >2.0 有被踢风险) / 角色特性点数 / 车辆无条件短接 / 服务器同步保护(防止属性/技能回滚) / 反检测(禁止默认日志记录器等)

### 信息

- 版本与反作弊状态(权限/移动, BikiniTools/自定义日志), 使用风险声明

### 设置

- 界面语言(切换即时生效) / 重载所有 Lua 元素 / 配置列表保存/加载/删除 / UI 强调色与玩家/载具/僵尸元素渲染颜色
