...
- **建号增强**: 建号全特性 / 技能满级 / 自带服装(创建新角色时生效)

### 物品

- **物品创建**: 名称/类别/ID 过滤, 一键给予 x1/x2/x5/x10
- **物品搜索 + 小地图标记**: 以玩家为中心 48 格、±1 层扫描已加载区块(家具/容器、地面与包、尸体、车辆容器), 命中以小地图灰色方块显示(同格多件计数); 标记随移动自动刷新; 小地图快捷开关栏(我/玩家/载具/僵尸/物品)与地图页勾选框双向同步
...
3. 从 `build` 目录中拿到 `EtherHack-3.2.0-B42.jar`。
...
产物在 `etherhack-src/build/EtherHack-3.2.0-B42.jar`。Lua 源码嵌入在 `src/main/resources/EtherHack/lua/` 中, 构建时自动打包。
...
| 路径 | 说明 |
|---|---|
| `etherhack-src/build/EtherHack-3.2.0-B42.jar` | 可直接使用的构建产物(当前版本) |
