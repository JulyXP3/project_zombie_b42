...
- Feature: attack speed multiplier — `IsoGameCharacter.calculateCombatSpeed()` return value is `FMUL`-multiplied by the multiplier (1.0-3.0, >2.0 carries a server kick risk); the input box moved to the right of the label with new "Apply"/"Reset" buttons for explicit commit and persistence (Reset restores default 1.0).
...
- Fixed: server sync protection (B42) end-to-end — 1) `ServerSyncBlocker`/`ISChatFix` are now loaded directly via `EtherLuaManager`/`RunLua` (vanilla PZ require can't resolve `EtherHack/lua` files, so the modules never loaded before); 2) filter timing fixed: in B42 `GameClient.update()` processes packets before `OnTick`, so filtering is now ASM-injected at the top of `GameClient.update()` (both `MainLoopNetData` and `MainLoopNetDataQ` queues), attribute/XP rollbacks no longer occur; 3) filtered packets are discarded back to `ZomboidNetDataPool` (no pool degradation / steady allocation); 4) filter log throttled to one line per 30s; 5) when protection is on, Fitness level is synced to `CharacterStat.FITNESS` (`LevelPerk` only updates stats in debug mode, so player-editor level-ups had no effect); 6) `ISChatFix`'s `makeFade` override now passes through the new B42 `fraction` argument (kills the `__mul` log spam); 7) anti-cheat option disable value changed from 0 to 4 for B42 (0 is rejected by the `min=1` check, so anti-cheat was never actually disabled); 8) `FieldCache` now warns only once per missing field/method.
