...
public class LuaCompiler {
    ...
    public boolean isShouldLuaCompile(String var1) {
        ...
        boolean needContent = isModPath && (this.isBlockCompileLuaAboutmodcore || this.isBlockCompileLuaWithBadWords);
        String var10 = "";
        if (needContent) {
        ...
        if (this.isBlockCompileLuaAboutmodcore) {
    ...
    public void init() {
        Logger.printLog("Initializing LuaCompiler...");
    }

    public static LuaCompiler getInstance() {
        if (instance == null) {
            instance = new LuaCompiler();
        }
        return instance;
    }
}
