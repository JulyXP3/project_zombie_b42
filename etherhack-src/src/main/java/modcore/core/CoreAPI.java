...
import modcore.core.RecipeAPI;
import modcore.core.ServerSyncBlocker;
import modcore.GameClientWrapper;
import modcore.annotations.LuaEvents;
import modcore.annotations.SubscribeLuaEvent;
import modcore.utils.ColorUtils;
import modcore.utils.ConfigUtils;
import modcore.utils.EventSubscriber;
import modcore.utils.Exposer;
...
public class CoreAPI {
    ...
    public static File configDir() {
        File dir = new File(zombie.core.Core.getInstance().getMyDocumentFolder(), "modcore/config");
        if (!dir.exists()) {
            dir.mkdirs();
            // 一次性迁移: 游戏目录下的旧配置 (3.2.4 早期版放游戏根) 搬到用户目录
            File legacy = new File("modcore/config");
    ...
    public void saveConfig(String var1) {
        ...
        var3.setProperty("isBlockCompileLuaWithBadWords", Boolean.toString(LuaCompiler.getInstance().isBlockCompileLuaWithBadWords));
        var3.setProperty("isBlockCompileLuaAboutmodcore", Boolean.toString(LuaCompiler.getInstance().isBlockCompileLuaAboutmodcore));
        var3.setProperty("isBlockCompileDefaultLua", Boolean.toString(LuaCompiler.getInstance().isBlockCompileDefaultLua));
    ...
    private void initStartupConfig() {
        File startupFile = new File(configDir(), "startup.properties");
        ...
        try (FileInputStream var2 = new FileInputStream(startupFile);){
            var1.load(var2);
        }
    ...
    public void loadAPI() {
        Logger.printLog("Loading CoreAPI...");
        // L1 修正: 捕获窗 2 — 全部 expose* 注入的方法名
    ...
    private void updateVehiclesVisuals() {
        ...
        if (this.isVisualsEnable && (this.isVisualsVehiclesEnable || this.isVisualDrawLineToVehicle) && (var1 = IsoPlayer.getInstance()) != null) {
            ...
            if (var2 != null && !var2.isEmpty()) {
                ...
                String vehiclePowerLabel = this.isVisualsVehiclesEnable ? CoreMain.getInstance().LuaTranslator.getTranslate("UI_VisualsDraws_VehiclePower") : null;
                String vehicleSpeedLabel = this.isVisualsVehiclesEnable ? CoreMain.getInstance().LuaTranslator.getTranslate("UI_VisualsDraws_VehicleSpeed") : null;
    ...
    private void updatePlayersVisuals() {
        ...
        if (this.isVisualsEnable && (this.isVisualDrawPlayerNickname || this.isVisualDrawPlayerInfo || this.isVisualDrawLineToPlayers) && (var1 = IsoPlayer.getInstance()) != null) {
            ...
            if (var2 != null && !var2.isEmpty()) {
                float scale = Rendering.espTextScale();
                int lineH = Rendering.getEspLineH();
                String primaryHandLabel = this.isVisualDrawPlayerInfo ? CoreMain.getInstance().LuaTranslator.getTranslate("UI_VisualsDraws_PrimaryHand") : null;
                String secondaryHandLabel = this.isVisualDrawPlayerInfo ? CoreMain.getInstance().LuaTranslator.getTranslate("UI_VisualsDraws_SecondaryHand") : null;
    ...
    extends Exposer {
        final /* synthetic */ CoreAPI this$0;

        public SafeExposer(CoreAPI CoreAPI, KahluaConverterManager m, Platform p, KahluaTable e) {
            CoreAPI etherAPI2 = CoreAPI;
            Objects.requireNonNull(etherAPI2);
            this.this$0 = etherAPI2;
            super(m, (J2SEPlatform)p, e);
        }

        public void exposeAPI(LuaMethods methods) {
        ...
        // 自动驾驶域 (modcore/drive, 研判 §八): 静态 @LuaMethod → 全局函数;
        // 引导时顺带读回持久化配置 (cruiseSpeed/policy, modcore/config/drive.properties)。
        public void exposeAutoDrive() {
            modcore.drive.AutoDriveAPI.loadConfig();
            for (Method method : modcore.drive.AutoDriveAPI.class.getMethods()) {
                if (!method.isAnnotationPresent(LuaMethod.class)) continue;
                LuaMethod annotation = method.getAnnotation(LuaMethod.class);
                String name = annotation.name();
                if (name == null || name.isEmpty()) {
                ...
                this.exposeGlobalClassFunction(LuaManager.env, modcore.drive.AutoDriveAPI.class, method, name);
                Logger.printLog("Exposed AutoDriveAPI method: " + name);
            }
        }
    ...
    extends LuaMethods {
        ...
        public SafeEtherLuaMethods(CoreAPI this$0) {
            CoreAPI CoreAPI = this$0;
            Objects.requireNonNull(CoreAPI);
            this.this$0 = CoreAPI;
        }
