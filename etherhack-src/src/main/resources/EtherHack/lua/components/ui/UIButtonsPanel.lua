...
function UIButtonsPanel:openPanel(id)
    ...
    if panel ~= nil and panel.localPlayer ~= getPlayer() then
        UIButtonsPanel.panelCache[panelById] = nil;
        panel = nil;
    end
    if panel == nil then
        panel = panelById:new(EtherTheme.framePad + self.width, EtherTheme.headerH, self.parent.width - self.width - EtherTheme.framePad * 2, self.parent.height - EtherTheme.headerH - EtherTheme.framePad);
        panel:initialise();
        panel:instantiate();
        UIButtonsPanel.panelCache[panelById] = panel;
    end
