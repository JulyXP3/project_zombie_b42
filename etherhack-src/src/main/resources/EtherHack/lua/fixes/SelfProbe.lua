...
--*   3. getLoadedLuaCount()+getLoadedLua(i) 枚举 → 应无 EtherHack 条目
--*      (L3 fileless; 签名与 KWRR 同款: count + 按索引取)
--**********************************************************

-- KWRR checkEtherHack 硬编码符号表 (OverrideClient.lua:118-215)
        ...
        if ok and type(count) == "number" then
            ...
            for i = 0, count - 1 do
                ...
                if okGet and type(f) == "string"
                    and string.find(f, "EtherHack", 1, true) ~= nil then
                    table.insert(fileHits, f);
                end
            end
            if #fileHits > 0 then
                print("[SelfProbe] FAIL-L3: EtherHack lua visible in getLoadedLua: "
                    .. table.concat(fileHits, ", "));
            else
                print("[SelfProbe] OK-L3: no EtherHack files in getLoadedLua ("
