...
--* (分析文档: 临时武器-分析与设计(已实施完成).md; B42 stable 客户端本地枪可开火,
...
--*   3. 武器数值一律真实脚本 (原判据 "AntiCheatHitWeapon 真实数值天然全过" 已被
--*      2026-09-15 一百零五轮取证更正: 命中包在服务端 isConsistent 阶段就被丢弃
--*      — Weapon 字段从玩家服务端背包解析不到本地枪 → 每枪 WARN + 包被丢,
--*      根本没走到反作弊 ⇒ MP 下本功能无服务端伤害, 纯本地表演; 详见分析文档
--*      "2026-09-15 一百零五轮取证补充与更正" 段);
...
function EtherTempWeapon.buildList()
    ...
    if not ok then
        print("[modcore] TempWeapon buildList error: " .. tostring(err));
        return;
    end
